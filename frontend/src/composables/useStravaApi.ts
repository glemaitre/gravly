...
export function useStravaApi() {
  ...
  async function handleAuthenticationError(): Promise<void> {
    console.info('Authentication error detected, attempting token refresh...')
  ...
  async function getActivities(
    ...
    try {
      ...
      const data = await response.json()
      console.info(`Retrieved ${data.activities?.length || 0} Strava activities`)
  ...
  async function initializeAuth(): Promise<void> {
    ...
    if (authState.value.isAuthenticated && authState.value.expiresAt) {
      ...
      if (timeUntilExpiry < fiveMinutes && timeUntilExpiry > 0) {
        console.info('Token expires soon, attempting proactive refresh...')
        try {
          await handleAuthenticationError()
        } catch (error) {
