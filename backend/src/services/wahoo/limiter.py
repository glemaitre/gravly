"""Rate limiter classes for the Wahoo API.
...
Requests every 5 minutes: 	25 	            200
...
from .protocol import RequestMethod
...
def get_rates_from_response_headers(
    ...
    if usage_rates and limit_rates:
        return RequestRate(
            ...
            middle_usage=usage_rates[1],
            long_usage=usage_rates[2],
            short_limit=limit_rates[0],
            middle_limit=limit_rates[1],
            long_limit=limit_rates[2],
        )
    else:
        return None
...
class SleepingRateLimitRule:
    ...
    def __init__(
        ...
        if priority not in ["low", "medium", "high"]:
            raise ValueError(
                f'Invalid priority "{priority}", expecting one of "low", "medium" '
                f'or "high"'
            )
